...
The amount of lines doubles.

* What happens if you change the output file name (after `>>`) to `~/MBB243/out/genotype_head.txt`? Why do you think that is?
The file can not be found as there out is not in the folder so the code can not put the information in to thetext file.
```{bash, engine.opts='-l'}
...
The amount of Hello terminal! increases in the EMPTY.txt
